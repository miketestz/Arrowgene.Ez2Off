...
{
    public class LobbyCharacterListPacket
    {
        public static IBuffer Create(Channel channel)
        {
            ...
            {
                Write(buffer, clients[i].Character, i);
            }

            return buffer;
        }

        public static void Write(IBuffer buffer, Character character, int index)
        {
            buffer.WriteInt16((short)index, Endianness.Big);
            buffer.WriteFixedString(character.Name, 16, Utils.KoreanEncoding);
            buffer.WriteByte(0x00); // String Termination
